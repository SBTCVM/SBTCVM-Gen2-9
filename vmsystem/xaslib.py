        ...
        for line in plugfile:
            ...
            if ';' in line:
                try:
                    ...
                    xascmds.extend(
                        [xascmd(xcmd, execstr, ispython, takesfile)])
                except IndexError:
                    continue
...
cfg_header_info = "#SBTCVM XAS Shell " + xasvers + \
            ...
            if os.path.isdir(joinedpath):
                ...
                if filen.startswith("r_"):
                    ...
                    for filesub in os.listdir(joinedpath):
                        ...
                        if filesub.startswith(
                                "auto_") and search.lower() in filen.lower():
                            if "." in filesub:
                                filesub_ext = filesub.split(".")[1].lower()
                                if filesub_ext in autodirexts:
                                    autodstr = (
                                        autodstr + filesub_ext.upper() + ", ")
                        else:
                            matcherprint(
                                filesub, search, filen + "+" + filesub, sublist=1)
                    if autodstr != "":
    ...
    if os.path.isdir(realpath):
        ...
        for smartd in iofuncts.smartpaths:
            if os.path.isdir(os.path.join(smartd, realpath)):
                showlisting(os.path.join(smartd, realpath), smartd.replace(
                    "\\", "+").replace("/", "+") + "+" + pathdesc)
                return
    print("XAS ERROR: path not found.")


# Debug command. (Intended to help debug interactive mode as it becomes
# more advanced.)
def debugcmd(arg):
    global shelldebug_plugcmdid
    if arg == "options":
        print(
            """DEBUG COMMANDS: (BOOL options accept 1/true/on and 0/false/off)
...
def configcmd(arg):
    ...
    if dcmd == "prompt":
        if darg == "":
            ...
            print(
                "Haha... Now seriously... Please specify 'config prompt [prompt text]'")
    ...
    while True:
        ...
        if cmd == "xas":
            if arg is not None:
                ...
                print(
                    "XAS ERROR: no argument after command: xas. Line: '" +
                    str(lineno) +
                    "'")
        elif cmd == "print":
            print(arg)
            ...
            for cmdobj in xascmds:
                if cmd == cmdobj.xcmd:
                    ...
                    if cmdobj.ispython:
                        ...
                        if cmdobj.takesfile and arg is not None:
                            if shelldebug_plugcmdid:
                                print(
                                    "plugin cmd: '" +
                                    cmd +
                            ...
                            if call(
                                ['python'] +
                                cmdobj.execstr.split(" ") +
                                    arg.split(" ")) != 0:
                                print(
                                    "XAS ERROR: plugin command error! cmd:'" + cmd + "'")
                            # print(shellwelcome)
                        else:
                            if shelldebug_plugcmdid:
                                print(
        ...
        if cmd == "xas":
            if arg is not None:
                ...
                if pathx != scrpath:
                    ...
                    print(
                        ppx +
                        "XAS ERROR: Subscript loop error. Line: '" +
                        str(lineno) +
                        "' in: '" +
                        scrpath +
                        "'")
                ...
                if cmdobj.ispython:
                    if cmdobj.takesfile and arg is not None:
                        ...
                        if call(
                            ...
                            print(
                                ppx +
                                "XAS ERROR: plugin command error! cmd:'" +
                                cmd +
                                "' Line: '" +
                                str(lineno) +
                                "' in: '" +
