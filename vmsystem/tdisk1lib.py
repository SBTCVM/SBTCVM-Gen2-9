...
def loaddisk(filename, readonly=0):
    ...
    try:
        ...
        if filecount > len(diskdat) - 1:
            return "Disk reports a file count '" + \
                str(filecount) + "' that is higher than actual number of files '" + str(len(diskdat) - 1) + "'"

        if filecount < len(diskdat) - 1:
            return "Disk reports a file count '" + \
                str(filecount) + "' that is lower than actual number of files '" + str(len(diskdat) - 1) + "'"

        if len(diskdat) - 1 > fcountmax:
            return "Disk contains more than 243 files: '" + \
                str(len(diskdat) - 1) + "'"
        filedict = {}
            ...
            for line in diskfile.split("\n"):
                if linecount == 0:
                    ...
                    if len(diskfile.split("\n")) > fsizemax:
                        return "File '" + dfile + "': Is beyond 39366 Nonets! '" + \
                            str(len(diskfile.split("\n")) * 2) + "'"
                elif line == "":
                    pass
                else:
                    try:
...
def savedisk(diskobj):
    ...
    if diskobj.rd:
        raise ValueError(
            "Disk Object is a ramdisk. It contains no filename to save to.")
    if diskobj.fname is None:
        raise ValueError(
            "Disk Object has 'None' for the filename. This is ONLY permitted for ramdisks!")
