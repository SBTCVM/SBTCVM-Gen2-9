...
class io:
    ...
    # ioread and iowrite should only be used by the CPU(s) devices should use
    # deviceread and devicewrite (see below)

    def ioread(self, addr):
        if int(addr) in self.ReadOverrideList:
            # get data at address anyways. (useful for refrence, ect, also
            # needed to keep IO map updated)
        ...
        # if address is registered by a component via setreadnotify, call the
        # specified function.
    ...
    def iowrite(self, addr, data):
        ...
        # if address is registered by a component via setwritenotify, call the
        # specified function.
    ...
    # that needs to read/write the IObus without triggering the read and write
    # function callbacks.
