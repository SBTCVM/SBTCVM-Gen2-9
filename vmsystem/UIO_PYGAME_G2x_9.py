...
class uio:
    def __init__(
        ...
        self.plotobj = PlotterEngine(
            ioref, 243, 243, self.plotx, self.ploty, 30)

        self.plotx31 = 575
        self.ploty31 = 575

        # load and set window icon.
        self.picon = pygame.image.load(
            os.path.join(*["vmsystem", "GFX", "icon64.png"]))
        pygame.display.set_icon(self.picon)

        # Init screen for TTY output. basing size on font properties.
        self.screensurf = pygame.display.set_mode(
            (charwidth * self.maxx, charheight * self.maxy))
        self.gamode = 0
        ...
        # 3 packed art colors as 3, 3-trit RGB values.
        ioref.setwritenotify(7, self.setpackcol)
        # same format as above, but dump 3 raw pixels instead.
        ioref.setwritenotify(8, self.colorpack)
        ...
        self.charcache = {}
        self.colorpackdict = {}
        self.backfill = monofont.render(
            " ", True, self.TTYbg, self.TTYbg).convert()

        self.gfxpakp = monofont.render(
            " ", True, (255, 255, 255), (255, 255, 255)).convert()
        self.gfxpak0 = monofont.render(
            " ", True, (127, 127, 127), (127, 127, 127)).convert()
        self.gfxpakn = monofont.render(
            " ", True, (0, 0, 0), (0, 0, 0)).convert()
        self.ttybuff = []
        self.pchar = ""
        self.charx = 0
        self.chary = charheight * (self.maxy - 1)
        self.newline_rect = pygame.Rect(
            0, self.chary, charwidth * self.maxx, charheight)
        ####
        self.newmode = None
        self.shutdown = 0
        self.clock = pygame.time.Clock()
        pygame.display.set_caption(
            romname + " - SBTCVM Gen2-9",
            romname + " - SBTCVM Gen2-9")
                ...
                if event.type == pygame.KEYDOWN:
                    ...
                    if event.key == pygame.K_RETURN and mods & pygame.KMOD_ALT:
                        if self.fscreen:
                            ...
                            self.screensurf = pygame.display.set_mode(
                                (self.screensurf.get_width(), self.screensurf.get_height()))
                            ...
                            scbak = self.screensurf.copy()
                            self.screensurf = pygame.display.set_mode(
                                (self.screensurf.get_width(), self.screensurf.get_height()), pygame.FULLSCREEN)
                            self.screensurf.blit(scbak, (0, 0))
                            pygame.display.flip()
    ...
    def dogamode(self, data):
        ...
        if self.fscreen:
            if data == 0:
                self.gamode = 0
                self.screensurf = pygame.display.set_mode(
                    (charwidth * self.maxx, charheight * self.maxy), pygame.FULLSCREEN)
            ...
            if data == 30:
                self.gamode = 30
                self.screensurf = pygame.display.set_mode(
                    (self.plotx, self.ploty), pygame.FULLSCREEN)
                self.plotobj.enable()
            ...
            if data == 31:
                self.gamode = 31
                self.screensurf = pygame.display.set_mode(
                    (self.plotx31, self.ploty31), pygame.FULLSCREEN)
                self.plotobj.enable()
                self.screensurf.fill((127, 127, 127))
                pygame.display.flip()
                self.mousesys.modechange(data)
    ...
    def newpackcolor(self, newcol):
        ...
        self.gfxpakp = monofont.render(
            " ", True, self.pakpcol, self.pakpcol).convert()
        self.gfxpak0 = monofont.render(
            " ", True, self.pak0col, self.pak0col).convert()
        self.gfxpakn = monofont.render(
            " ", True, self.pakncol, self.pakncol).convert()
    ...
    def ttywrite(self, addr, data):
        if data == 1:
            ...
            if self.xttycharpos != 0:
                self.xttycharpos -= 1
                # Ensure previous character is actually removed in shell
                # output.
    ...
    def charrender(self, char):
        if char + self.colorkey in self.charcache:
            ...
            chtx = monofont.render(
                char, True, self.textfg, self.textbg).convert()
        ...
        while len(self.ttybuff) > 0 and dcount != 90:
            ...
            if self.charx >= self.ttywide and char != "\n":
                if self.linebgfill == 1:
                    self.linebgfill = 0
                    pxrect = pygame.Rect(
                        self.charx, self.chary, charwidth * self.maxx, charheight)
            ...
            if isinstance(char, list):
                # When list code 20 is detected, change colors using helper
                # method.
                if char[0] == 20:
                ...
                if char[0] == 30:
                    uprect = self.screensurf.blit(
                        self.char30(char[1]), (self.charx, self.chary))
            ...
            # backspace handler
                ...
                if self.charx != 0 and self.pchar != "\n" and self.pchar != "\b":
                    ...
                    self.charx -= charwidth
                    uprect = self.screensurf.blit(
                        self.backfill, (self.charx, self.chary))
                    if len(self.ttybuff) < 4:
                        uprects.append(
                            pygame.draw.line(
                                self.screensurf,
            ...
            # ternary packed art rendering
                ...
                uprect = self.screensurf.blit(
                    self.gfxpakn, (self.charx, self.chary))
                self.charx += charwidth
                uprects.append(uprect)
            elif char == 0:
                uprect = self.screensurf.blit(
                    self.gfxpak0, (self.charx, self.chary))
                self.charx += charwidth
                uprects.append(uprect)
            elif char == 1:
                uprect = self.screensurf.blit(
                    self.gfxpakp, (self.charx, self.chary))
                self.charx += charwidth
                uprects.append(uprect)
            else:
                # normal character handler
                uprect = self.screensurf.blit(
                    self.charrender(char), (self.charx, self.chary))
                if len(self.ttybuff) < 4:
                    uprects.append(
                        pygame.draw.line(
                            self.screensurf,
...
class MouseEngine:
    ...
    def mousedown_gaplot(self, event):
        posy = int(((event.pos[1] / float(self.plot.realy))
                   * self.plot.ysize) - self.plot.offset)
        posx = int(((event.pos[0] / float(self.plot.realx))
                   * self.plot.xsize) - self.plot.offset)
        button = event.button
    ...
    def mousedown_ga0(self, event):
        posx, posy = (event.pos[0] // self.TTYcharw,
                      event.pos[1] // self.TTYcharh)
        button = event.button
        self.clickbuff.append([button, posx, posy])
    ...
    def getrealx(self, addr, data):
        ...
        if self.gamode == 30:
            return btint(
                int(((mpos[0] / float(self.plot.realx)) * self.plot.xsize) - self.plot.offset))
    ...
    def getrealy(self, addr, data):
        ...
        if self.gamode in plotter_modes:
            return btint(
                int(((mpos[1] / float(self.plot.realy)) * self.plot.ysize) - self.plot.offset))
...
class PlotterEngine:
    ...
    def line(self, addr, data):
        self.drawbuff.append(
            [1, self.x1, self.y1, self.x2, self.y2, self.color])

    def tri(self, addr, data):
        self.drawbuff.append(
            [3, self.x1, self.y1, self.x2, self.y2, self.x3, self.y3, self.color])

    def rect(self, addr, data):
        self.drawbuff.append(
            [2, self.x1, self.y1, self.widthr, self.heightr, self.color])
    ...
    def draw(self, surface):
        ...
        while len(self.drawbuff) > 0 and cnt != self.itemlimit:
            ...
            if chunk[0] == 0:
                ...
                fullup = 1
            if chunk[0] == 1:
                uprects.append(pygame.draw.line(Xsurface,
                                                chunk[5],
                                                (int(chunk[1] * self.xmag),
                                                 int(chunk[2] * self.ymag)),
                                                (int(chunk[3] * self.xmag),
                                                    int(chunk[4] * self.ymag))))
            if chunk[0] == 2:
                drect = pygame.Rect((int(chunk[1] *
                                         self.xmag), int(chunk[2] *
                                                         self.ymag)), (int(chunk[3] *
                                                                           self.xmag), int(chunk[4] *
                                                                                           self.ymag)))
                uprects.append(pygame.draw.rect(Xsurface, chunk[5], drect, 0))
            if chunk[0] == 3:
                uprects.append(pygame.draw.polygon(Xsurface,
                                                   chunk[7],
                                                   [(int(chunk[1] * self.xmag),
                                                     int(chunk[2] * self.ymag)),
                                                       (int(chunk[3] * self.xmag),
                                                        int(chunk[4] * self.ymag)),
                                                       (int(chunk[5] * self.xmag),
                                                        int(chunk[6] * self.ymag))]))
            if chunk[0] == 4:
