...
def cocpu_setup(cpusys2, iosys1, iosys2, memsys2, targtime, targspeed):
    ...
    dispthr = Thread(
        target=cocpuinst.cocpu_process,
        args=[
            targtime * 2,
            targspeed / 2.0])
    dispthr.daemon = True
...
class cocpu:
    ...
    def cocpu_process(self, targtime, targspeed):
        while self.progrun:
            if self.cocpu_hlt == 1:
                ...
                self.clcnt += 1
                # project when the next cycle should start, then subtract
                # current time.
                xtime = (self.starttime + (self.clcnt - 1.0)
                         * targtime) - time.time()
                # sleep for remaining time (xtime) if it is above 0
                if xtime > 0.0:
                    time.sleep(xtime)
                # exit code:
                if retval is not None or self.shutdown == 1:
                    if retval is not None:

                        self.postout1 = ("COCPU VMSYSHALT " +
                                         str(retval[1]) + ": " + retval[2])
                    else:
                        self.postout1 = None
                    self.postout2 = ("COCPU Approx. Speed: '" +
                                     str((float(self.clcnt) /
                                          (time.time() -
                                           self.starttime)) /
                                         1000) +
                                     "' KHz")
                    self.postout3 = (
                        "COCPU Target Speed : '" + str(targspeed) + "' Khz")
    ...
    def printstats(self):
        if self.postout1 is not None:
            print(self.postout1)

        if self.postout2 is not None:
            print(self.postout2)

        if self.postout3 is not None:
            print(self.postout3)
