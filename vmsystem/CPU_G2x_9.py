...
class cpu:
    ...
    def cycle(self):
        ...
        if self.critfault is not None:
            ...
            if self.instval.intval != 100 and self.instval.intval != 101:
                ...
                self.normintreturns.insert(
                    0, [self.reg1.intval, self.reg2.intval, self.execpoint.intval, self.exceptcode])
                self.execpoint.intval = self.dataval.intval
                self.instval.intval = self.memsys.getinst(
                    self.execpoint).intval
                self.dataval.intval = self.memsys.getdata(
                    self.execpoint).intval
        ...
        # divmod
            ...
            try:
                self.reg2.intval, self.reg1.intval = divmod(
                    self.reg1.intval, self.reg2.intval)
                self.pointeroll1()
                self.pointeroll2()
            except ZeroDivisionError:
                self.reg2.intval = 0
    ...
    def exreturn(self):
        try:
            self.reg1.intval, self.reg2.intval, self.execpoint.intval, self.exceptcode = self.exintreturns.pop(
                0)
            if self.exceptcode != 0:
                self.exceptflag = 1
        except IndexError:
            return
    # clears the Exception status, but keeps going at current address, and
