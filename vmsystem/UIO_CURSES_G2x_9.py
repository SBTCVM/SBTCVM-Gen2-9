...
class uio:
    ...
    def statup(self):
        ...
        while self.run:
            self.maxy = self.ttywin.getmaxyx()[0]
            self.statwin.erase()
            self.statwin.addstr("r1:" +
                                str(self.cpu.reg1.intval) +
                                ", r2:" +
                                str(self.cpu.reg2.intval) +
                                ", in:" +
                                str(self.cpu.instval.intval) +
                                ", da:" +
                                str(self.cpu.dataval.intval) +
                                ", ex:" +
                                str(self.cpu.execpoint.intval))
            self.statwin.refresh()
            self.ttywin.refresh()
            time.sleep(0.05)
            # todo: Get Function & special key input parsing working, and add
            # IO line for TTY read.
            try:
                ...
                if keyinp in tcon.strtodat:
                    ...
                    self.keyinbuff.append(
                        tcon.strtodat[tcon.curses_specials[keyinp]])
            except curses.error:
                continue
        self.running = 0
        return
