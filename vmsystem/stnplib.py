...
def fsyntax_break(lineno, info):
    if fsyntax_stack == []:
        return 1, "Break: '" + info + "' on line '" + \
            str(lineno) + "', is not within a block."


def fsyntax_top(lineno, info):
    if fsyntax_stack == []:
        return 1, "End: '" + info + "' on line '" + \
            str(lineno) + "', is not within a block."
    ...
    def p0(self, args, keyword, lineno):
        ...
        for char in name:
            if char not in varvalid:
                return 1, keyword + ": Line: " + \
                    str(lineno) + ": Invalid character in variable name! '" + char + "'"
        if name in reservednames:
            return 1, keyword + ": Line: " + \
                str(lineno) + ": variable name: '" + args + "' Is reserved."
        if data.replace("@", "10x").startswith("10x"):
            try:
                int(data.replace("@", "10x")[3:])
            except ValueError:
                return 1, keyword + ": Line: " + \
                    str(lineno) + ": decimal int syntax error!"
        # this syntax will make this var equal the encoding data of the
        # specified character.
        elif data.startswith(":"):
            if len(data) < 2:
                return 1, keyword + ": Line: " + \
                    str(lineno) + ": Must specify character"
        else:
            if len(data.replace("*", "")) > 9:
                return 1, keyword + ": Line: " + \
                    str(lineno) + ": string too large!"
            for char in data.replace("*", ""):

                if char not in tritvalid:
                    return 1, keyword + ": Line: " + \
                        str(lineno) + ": invalid char in ternary data string!"
        return 0, None
...
class in_const:
    ...
    def p_const(self, args, keyword, lineno):
        ...
        for char in name:
            if char not in varvalid:
                return 1, keyword + ": Line: " + \
                    str(lineno) + ": Invalid character in constant name! '" + char + "'"
        if data.replace("@", "10x").startswith("10x"):
            try:
                int(data.replace("@", "10x")[3:])
            except ValueError:
...
class in_include:
    ...
    def pmacro(self, args, keyword, lineno):
        ...
        try:
            ...
            return 1, keyword + ": Line: " + \
                str(lineno) + ": must specify 'include [modulename] as [varspace name]"
        print(
            "MODULE LOAD: PRESCAN: LINE " +
            str(lineno) +
            ": " +
            basename +
            " as " +
        ...
        if varprefix == "":
            return 1, keyword + ": Line: " + \
                str(lineno) + ": varspace name MUST NOT BE BLANK"
        if varprefix in self.varspacenames:
            return 1, keyword + ": Line: " + \
                str(lineno) + ": varspace name '" + varprefix + "' already used. DUPLICATES NOT ALLOWED."
        for char in varprefix:
            if char not in varvalid:
                return 1, keyword + ": Line: " + \
                    str(lineno) + ": Invalid character in varspace name! '" + char + "'"

        self.varspacenames.extend([varprefix])
        # file existence checks:
        if iofuncts.findtrom(
                basename,
                ext=".tas0",
                exitonfail=0,
                dirauto=1) is None:
            return 1, keyword + ": Line: " + \
                str(lineno) + ": '" + basename + "': UNABLE TO FIND MODULE's '*.tas0' ASSEMBLER MODULE FILE."
        if iofuncts.findtrom(
                basename,
                ext=".stnpmfs",
                exitonfail=0,
                dirauto=1) is None:
            return 1, keyword + ": Line: " + \
                str(lineno) + ": '" + basename + "': UNABLE TO FIND MODULE's '*.stmpmfs' VARIABLE MANIFEST FILE."

        # check manifest file for errors/corrupted data during first error
        # check pass.
        fname = iofuncts.findtrom(
            basename,
            ext=".stnpmfs",
            exitonfail=0,
            dirauto=1)
                ...
                try:
                    ...
                    if flist[0] == "macro":
                        try:
                            ...
                            return 1, keyword + ": Line: " + \
                                str(lineno) + ": '" + basename + "': MALFORMED MACRO ENTRY IN MANIFEST\n    '" + f + "'"
                        nvar = npvar(name, code, vtype=nptype_macro)
                        nvar.modprefix = varprefix + "."
                        varlist.extend([nvar])
                except IndexError:
                    return 1, keyword + ": Line: " + \
                        str(lineno) + ": '" + basename + "': MALFORMED DATAFIELD IN MANIFEST\n    '" + f + "'"
        fileobj.close()
        return varlist
        ...
        for f in fileobj:
            ...
            if ";" in f:
                try:
                    ...
                    if flist[0] == "const":
                        varlist.extend(
                            [npvar(varprefix + "." + flist[1], flist[2], vtype=nptype_const, frommodule=1)])

                    if flist[0] == "label":
                        npvar(flist[1], None, vtype=nptype_label)
                    if flist[0] == "table":
                        try:
                            npvar(flist[1], [int(flist[2]), int(
                                flist[3])], vtype=nptype_table)
                        except ValueError:
                            return 1, keyword + ": Line: " + \
                                str(lineno) + ": '" + basename + "': MALFORMED TABLE SIZE FIELD IN MANIFEST\n    '" + f + "'"
                except IndexError:
                    return 1, keyword + ": Line: " + \
                        str(lineno) + ": '" + basename + "': MALFORMED DATAFIELD IN MANIFEST\n    '" + f + "'"
        fileobj.close()
    ...
    def p1(self, args, keyword, lineno):
        ...
        for f in fileobj:
            ...
            if ";" in f:
                ...
                if flist[0] == "int9":
                    varlist.extend(
                        [npvar(varprefix + "." + flist[1], flist[2], vtype=nptype_int, frommodule=1)])
                # if flist[0]=="const":
                # varlist.extend([npvar(varprefix + "." + flist[1], flist[2], vtype=nptype_const, frommodule=1)])
                if flist[0] == "label":
                    varlist.extend(
                        [npvar(varprefix + "." + flist[1], None, vtype=nptype_label, frommodule=1)])
                if flist[0] == "table":
                    varlist.extend([npvar(
                        varprefix + "." + flist[1], [flist[2], flist[3]], vtype=nptype_table, frommodule=1)])
        fileobj.close()
...
class in_label:
    ...
    def p0(self, args, keyword, lineno):
        for char in args:
            if char not in varvalid:
                return 1, keyword + ": Line: " + \
                    str(lineno) + ": Invalid character in label! '" + char + "'"
        if args in reservednames:
            return 1, keyword + ": Line: " + \
                str(lineno) + ": label name: '" + args + "' Is reserved."
        return 0, None
...
class in_table:
    ...
    def p0(self, args, keyword, lineno):
        try:
            ...
            return 1, keyword + ": Line: " + \
                str(lineno) + ": MUST SPECIFY [name],[width],[height] as arguments."

        try:
            w = int(w)
            if w < 0:
                return 1, keyword + ": Line: " + \
                    str(lineno) + ": Invalid width argument. must be positive."
        except ValueError:
            return 1, keyword + ": Line: " + \
                str(lineno) + ": Invalid width argument. must be decimal."
        ...
        for char in args:
            if char not in varvalid:
                return 1, keyword + ": Line: " + \
                    str(lineno) + ": Invalid character in table! '" + char + "'"
        if args in reservednames:
            return 1, keyword + ": Line: " + \
                str(lineno) + ": table name: '" + args + "' Is reserved."
        return 0, None
...
class in_marker:
    ...
    def p0(self, args, keyword, lineno):
        if args == "":
            return 1, keyword + ": Line: " + \
                str(lineno) + ": Unnamed Debug Marker!"
        return 0, None
...
class in_rawasm:
    ...
    def p3(
            ...
            destobj):
        destobj.write(
            "#___RAW ASSEMBLY CODE___\n#_______NOTE: this corresponds to SSTNPL source line #" +
            str(lineno) +
            "\n" +
            args +
            "#SSTNPL Source Line: '" +
...
class in_intcommon1:
    ...
    def p2(self, args, keyword, lineno, nvars, valid_nvars, labels, tables):
        if args in valid_nvars:
            ...
            return 1, keyword + ": Line: " + \
                str(lineno) + ": Nonexistant variable'" + args + "'"
...
class in_for:
    ...
    def p0(self, args, keyword, lineno):
        try:
            ...
            return 1, keyword + ": Line: " + \
                str(lineno) + ": invalid argument sequence.'" + args + "'"
        try:
        ...
        if formode not in ["urange", "drange", "random"]:
            return 1, keyword + ": Line: " + \
                str(lineno) + ": '" + formode + "' is not a valid for mode."
        ...
        for char in var:
            if char not in varvalid:
                return 1, keyword + ": Line: " + \
                    str(lineno) + ": Invalid character in iteration variable name! '" + char + "'"
        if var in reservednames:
            return 1, keyword + ": Line: " + \
                str(lineno) + ": iteration variable name: '" + var + "' Is reserved."
    ...
    def p2(self, args, keyword, lineno, nvars, valid_nvars, labels, tables):
        for vint in args.split(","):
            if vint in valid_nvars:
                ...
                return 1, keyword + ": Line: " + \
                    str(lineno) + ": Nonexistant variable'" + vint + "'"
        return 0, None
...
class in_tabr:
    ...
    def p0(self, args, keyword, lineno):
        try:
            ...
            return 1, keyword + ": Line: " + \
                str(lineno) + ": Must Specify [name],[xvar],[yvar] as arguments."
        if isaliteral(yv):
    ...
    def p2(self, args, keyword, lineno, nvars, valid_nvars, labels, tables):
        ...
        if xv not in valid_nvars:
            return 1, keyword + ": Line: " + \
                str(lineno) + ": Nonexistant variable'" + xv + "'"
        if yv not in valid_nvars:
            return 1, keyword + ": Line: " + \
                str(lineno) + ": Nonexistant variable'" + yv + "'"
        if tname not in tables:
            return 1, keyword + ": Line: " + \
                str(lineno) + ": Nonexistant table name '" + tname + "'"
        return 0, None
...
class in_tabw:
    ...
    def p0(self, args, keyword, lineno):
        try:
            ...
            return 1, keyword + ": Line: " + \
                str(lineno) + ": Must Specify [name],[xvar],[yvar],[datavar] as arguments."
        if isaliteral(yv):
            xret = literal_syntax(yv, keyword, lineno)
            if xret is not None:
                return xret
    ...
    def p2(self, args, keyword, lineno, nvars, valid_nvars, labels, tables):
        ...
        if datav not in valid_nvars:
            return 1, keyword + ": Line: " + \
                str(lineno) + ": Nonexistant variable'" + datav + "'"
        if yv not in valid_nvars:
            return 1, keyword + ": Line: " + \
                str(lineno) + ": Nonexistant variable'" + yv + "'"
        if tname not in tables:
...
class in_mulpack:
    ...
    def p0(self, args, keyword, lineno):
        ...
        for arg in arglist:
            if len(arg) != 9:
                return 1, keyword + ": Line: " + \
                    str(lineno) + ": Pack Chunk '" + arg + "' Not 9 trits."
            for char in arg:
                if char not in tritvalid:
                    return 1, keyword + ": Line: " + \
                        str(lineno) + ": Invalid character in Pack Chunk '" + arg + "': '" + char + "'"
        return 0, None
...
class in_labelgoto:
    ...
    def p2(self, args, keyword, lineno, nvars, valid_nvars, labels, tables):
        if args in labels:
            ...
            return 1, keyword + ": Line: " + \
                str(lineno) + ": Nonexistant label'" + args + "'"
...
goto;>goto--branch-''' + str(lineno)
    ...
    def p0(self, args, keyword, lineno):
        ...
        if len(arglist) != 3:
            if len(arglist) != 2:
                return 1, keyword + ": Line: " + \
                    str(lineno) + ": Must specify args as '<var>,<var> goto <label>'"
        ...
        if self.condmode in [2, 3]:
            try:
                ...
                return 1, keyword + ": Line: " + \
                    str(lineno) + ": Must specify range condition args as '<startvar>,<endvar>,<valuevar>'"
        else:
            ...
            try:
                ...
                return 1, keyword + ": Line: " + \
                    str(lineno) + ": Must specify condition args as '<var>,<var>'"
        # LITERALS
        if isaliteral(arga):
    ...
    def p2(self, args, keyword, lineno, nvars, valid_nvars, labels, tables):
        ...
        if len(arglist) == 3 and not arglist[1].startswith(
                "=") and not arglist[1] in ["chardump", "dumpt", "dumpd"]:
            label = arglist[2]
            if label not in labels:
                return 1, keyword + ": Line: " + \
                    str(lineno) + ": Nonexistant label'" + label + "'"
        if arglist[1].startswith("="):
            vname = arglist[1][1:]
            if vname not in valid_nvars:
                return 1, keyword + ": Line: " + \
                    str(lineno) + ": Nonexistant destination variable'" + vname + "'"
            valname = arglist[2]
            if valname not in valid_nvars:
                return 1, keyword + ": Line: " + \
                    str(lineno) + ": Nonexistant source variable'" + vname + "'"
        if arglist[1] in ["chardump", "dumpt", "dumpd"]:
            valname = arglist[2]
            if valname not in valid_nvars:
                return 1, keyword + ": Line: " + \
                    str(lineno) + ": Nonexistant character variable'" + vname + "'"
        # check goto mode
        if (arglist[1] not in ["begin",
                               "break",
                               "top",
                               "goto",
                               "gsub",
                               ...
                               "chardump",
                               "dumpt",
                               "dumpd"]) and (not arglist[1].startswith("=")):
            return 1, keyword + ": Line: " + \
                str(lineno) + ": Invalid conditional mode! See documentation for valid modes."
        # variable check
        for x in arglist[0].split(","):
            if x not in valid_nvars:
                return 1, keyword + ": Line: " + \
                    str(lineno) + ": Nonexistant variable'" + x + "'"
...
class in_int2opmath:
    ...
    def p0(self, args, keyword, lineno):
        ...
        if len(argsplit) != 2:
            return 1, keyword + ": Line: " + \
                str(lineno) + ": Two comma-separated variable/literal arguments required!"
        arga, argb = args.split(",")
        if isaliteral(arga):
            xret = literal_syntax(arga, keyword, lineno)
            if xret is not None:
    ...
    def p2(self, args, keyword, lineno, nvars, valid_nvars, labels, tables):
        ...
        for argx in argsplit:
            if argx in valid_nvars:
                ...
                return 1, keyword + ": Line: " + \
                    str(lineno) + ": Nonexistant variable'" + argx + "'"
...
# random number implementation based upon SBTCVM psudo-random number
# generator 'rand1'.
class in_rrange:
    def __init__(self):
    ...
    def p0(self, args, keyword, lineno):
        ...
        if len(argsplit) != 2:
            return 1, keyword + ": Line: " + \
                str(lineno) + ": Two comma-separated variable arguments required!"
        arga, argb = args.split(",")
...
# Active equivalent to getchar. will only continue on a non-null character
# from TTY.
class in_keyprompt:
    def __init__(self):
...
setreg2;0
iowrite1;>io.ttyrd
ioread1;>io.ttyrd;keyprompt--loop-""" +
            str(lineno) +
            """
gotoif;>keyprompt--loop-""" +
            str(lineno) +
            """
""")
        return
...
class in_u2iter:
    ...
    def p0(self, args, keyword, lineno):
        try:
            ...
            return 1, keyword + ": Line: " + \
                str(lineno) + ": invalid argument sequence.'" + char + "'"
        for char in namex:
            if char not in varvalid:
                return 1, keyword + ": Line: " + \
                    str(lineno) + ": Invalid character in Iterator X State Integer variable! '" + char + "'"
        for char in namey:
            if char not in varvalid:
                return 1, keyword + ": Line: " + \
                    str(lineno) + ": Invalid character in Iterator Y State Integer variable! '" + char + "'"
    ...
    def p2(self, args, keyword, lineno, nvars, valid_nvars, labels, tables):
        ...
        if subx not in labels:
            return 1, keyword + ": Line: " + \
                str(lineno) + ": Nonexistant label'" + subx + "'"
        return 0, None
...
class in_uiter:
    ...
    def p0(self, args, keyword, lineno):
        ...
        for char in namex:
            if char not in varvalid:
                return 1, keyword + ": Line: " + \
                    str(lineno) + ": Invalid character in Iterator State Integer variable! '" + char + "'"
        if isaliteral(startx):
            xret = literal_syntax(startx, keyword, lineno)
            if xret is not None:
                return xret
...
class in_waitcycle:
    ...
    def p0(self, args, keyword, lineno):
        try:
            ...
            return 1, keyword + ": Line: " + \
                str(lineno) + ": invalid cycle count integer'" + args + "'"
        # value can't be any lower than 6, as the loop wouldn't run at all.
        if argint < 6:
            return 1, keyword + ": Line: " + \
                str(lineno) + ": '" + args + "' is less than the minimum supported cycle count (6)"

        # ensure value is at or below 9841 once divided by wait loop length (6)
        # aka 59046
        if argint > 59046:
            return 1, keyword + ": Line: " + \
                str(lineno) + ": '" + args + "' is greater than the maximum supported cycle count (59046)"

        if argint % 6 != 0:
            print("SUM: Notice: the requested wait time: '" +
                  args +
    ...
    def p0(self, args, keyword, lineno):
        for char in args:
            if char not in tcon.normal_char_list:
                return 1, keyword + ": Line: " + \
                    str(lineno) + ": invalid character'" + char + "'"
        return 0, None
...
class in_tpad:
    ...
    def p0(self, args, keyword, lineno):
        try:
            ...
            if intarg < 1:
                return 1, keyword + ": Line: " + \
                    str(lineno) + ": tpad size must be 1 or higher!" + arg + "'"
        except ValueError:
            return 1, keyword + ": Line: " + \
                str(lineno) + ": invalid tpad size integer'" + arg + "'"
        return 0, None
...
class in_tabstrc:
    ...
    def p0(self, args, keyword, lineno):
        ...
        if len(arglist) != 4:
            return 1, keyword + ": Line: " + \
                str(lineno) + ": Please specify arguments as: '[table],[xoffset],[ypos],[string]'"
        xoffset = arglist[1]
        ...
        if len(arglist[3]) == 0:
            return 1, keyword + ": Line: " + \
                str(lineno) + ": string must not be zerosize!"
    ...
    def p2(self, args, keyword, lineno, nvars, valid_nvars, labels, tables):
        ...
        if xoffset not in valid_nvars:
            return 1, keyword + ": Line: " + \
                str(lineno) + ": Nonexistant variable'" + xoffset + "'"
        if ypos not in valid_nvars:
            return 1, keyword + ": Line: " + \
                str(lineno) + ": Nonexistant variable'" + ypos + "'"
        if tname not in tables:
            return 1, keyword + ": Line: " + \
                str(lineno) + ": Nonexistant table name '" + tname + "'"
        return 0, None
...
class in_tdat:
    ...
    def p0(self, args, keyword, lineno):
        if args == "":
            return 1, keyword + ": Line: " + \
                str(lineno) + ": Blank table Error"
        arglist = args.split(";")
        for arg in arglist:
            sublist = arg.split(" ")
            if len(sublist) > 2:
                return 1, keyword + ": Line: " + \
                    str(lineno) + ": Invalid double.  '" + arg + "' please use format: 'arg1a arg1b;arg2a arg2b' etc."
            for subarg in sublist:
                if subarg.replace("@", "10x").startswith("10x"):
                    try:
                        ...
                        return 1, keyword + ": Line: " + \
                            str(lineno) + ": decimal int syntax error!: '" + subarg + "' "
                elif subarg.startswith(":"):
                    if len(subarg) < 2:
                        return 1, keyword + ": Line: " + \
                            str(lineno) + ": Must specify character: '" + subarg + "' "
                else:
                    if len(subarg.replace("*", "")) > 9:
                        return 1, keyword + ": Line: " + \
                            str(lineno) + ": string too lsubarge!: '" + subarg + "' "
                    for char in subarg.replace("*", ""):

                        if char not in tritvalid:
                            return 1, keyword + ": Line: " + \
                                str(lineno) + ": invalid char in ternary data string!: '" + subarg + "' "
        return 0, None
...
def literal_syntax(arg, keyword, lineno):
    if arg.startswith("$"):
        if arg[1:] not in constants:
            print(constants)
            return 1, keyword + ": Line: " + \
                str(lineno) + ": constant: '" + arg + "' not valid builtin/declared constant"
    if arg.startswith("@"):
        try:
            int(arg[1:])
        except ValueError:
            return 1, keyword + ": Line: " + \
                str(lineno) + ": Invalid static integer value '" + arg + "' Must use signed decimal."
    if arg.startswith(":"):
        if arg[1:] not in tcon.asm_chrtodat:
            return 1, keyword + ": Line: " + \
                str(lineno) + ": invalid character in literal'" + arg + "'"
    if arg.startswith("*"):
        for x in arg[1:]:
            if x not in tritvalid:
                return 1, keyword + ": Line: " + \
                    str(lineno) + ": invalid ternary literal'" + arg + "'"
    return None
...
def DO_buffer(num):
    return [in_common0(["brdhead" + num],
                       "ioread1;>buffer." + num + ".read.head\n",
                       "buffer " + num + " head read"),
            in_common0(["brdtail" + num],
                       "ioread1;>buffer." + num + ".read.tail\n",
                       "buffer " + num + " tail read"),
            in_common0(["breset" + num],
                       "iowrite1;>buffer." + num + ".reset\n",
                       "buffer " + num + " reset"),
            in_intcommon1b(["bwrhead" + num],
                           "dataread1;>",
                           "\niowrite1;>buffer." + num + ".write.head\n",
                           "buffer " + num + " head write"),
            in_intcommon1b(["bwrtail" + num],
                           "dataread1;>",
                           "\niowrite1;>buffer." + num + ".write.tail\n",
                           "buffer " + num + " tail write"),
            in_str_out(["bprinthead" + num],
                       ">buffer." + num + ".write.head",
                       newl=True),
            in_str_out(["bprinttail" + num],
                       ">buffer." + num + ".write.tail",
                       newl=True),
            in_str_out(["bprinthead" + num + "n"],
                       ">buffer." + num + ".write.head",
                       newl=False),
            in_str_out(["bprinttail" + num + "n"],
                       ">buffer." + num + ".write.tail",
                       newl=False)]
        ...
        try:
            ...
            for f in name:
                if f not in varvalid:
                    return 1, "def: (line " + str(lineno) + "): invalid character '" + \
                        f + "' in macro name '" + name + "'"
        except ValueError:
...
class mainloop:
        ...
        for line in self.srclines_macro:
            ...
            if line.startswith("!"):
                if not line.endswith(")"):
                    return 1, "Parser Error! (line " + str(lineno) + \
                        "): split line not terminated with closing ')' !"
                line = line[1:-1]

                args = line.split("(", 1)
                # check if name is specified & args are structured correctly
                if len(args) != 2:
                    return 1, "Macro Error! (line " + str(lineno) + \
                        "): must specify !name(arg,arg2,arg3...)!"
                ...
                if not len(args) == macro_args:
                    if not args == [""]:
                        return 1, "Macro Error! (line " + str(lineno) + "): Argument Mismatch! '" + str(
                            macro_args) + "' required by '" + name + "'."
                ...
                if "%mod%" in newline:
                    try:
                        newline = newline.replace(
                            "%mod%", macro_npvar.modprefix)
                    except AttributeError:
                        newline = newline.replace("%mod%", "")
                for f in args:
                    # allow for cannonical name(arg, arg, arg) form found in
                    # other languages.
                    if f.startswith(" "):
                        f = f[1:]
            ...
            if line.startswith("{"):
                if not line.endswith("}"):
                    # print(line)
                    return 1, "Parser Error! (line " + str(lineno) + \
                        "): split line not terminated with closing brace!"
                line = line[1:-1]
                line_subcount = 0
                for item in line.split(" / "):
                    line_subcount += 1
                    self.srclines.append(
                        [str(lineno) + "-" + str(line_subcount), item])
            else:
        ...
        for line in self.srclines:
            ...
            if keyword not in self.valid_instructs:
                if keyword == "":
                    ...
                    return 1, "Parser Error! (line " + str(lineno) + ") : '" + \
                        keyword + "' is not a valid SSTNPL keyword!"
        return 0, None
    ...
    # constant syntax prepass (used instead of pass 0 for in_include &
    # in_const)
    def p_const(self):
        # self.srcobj.seek(0)
        # lineno=0
    ...
    def p2(self):
        ...
        for line in self.srclines:
            ...
            for inst in self.instructs:
                if keyword in inst.keywords:
                    retval, errordesc = inst.p2(
                        data, keyword, lineno, self.nvars, self.valid_nvars, self.labels, self.tables)
                    if retval != 0:
                        return 1, errordesc
