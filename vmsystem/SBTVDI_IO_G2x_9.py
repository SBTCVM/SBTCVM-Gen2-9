...
class sbtvdi:
    def __init__(
        ...
        # CLI mode    : mirrors user input, intened to be used directly by
        # user.
        self.prm = 0
...
''')
            self.outstr(
                '''dmnt0 [disk image]: Mount SBTVDI disk image to drive index 0
...
''')
            ...
            if not len(cmdlist_as) >= 2:
                ...
                fname = iofuncts.findtrom(
                    cmdlist_as[1], ext=".tdsk1", exitonfail=0, dirauto=1)
                # print(fname)
                if fname is None:
                    self.outstr(
                        "ERROR: disk image '" +
                    ...
                    if isinstance(retval, str):
                        ...
                        if self.disks[mntindex] is not None:
            ...
            if not len(cmdlist_as) >= 3:
                ...
                try:
                    if int(
                            cmdlist_as[1]) in self.disks or int(
                            cmdlist_as[1]) == -1:
                        self.membackup(int(cmdlist_as[1]), cmdlist_as[2])
                except ValueError:
                    self.outstr(
                        "ERROR: Invalid Integer in disk id! '" +
                        cmdlist_as[1] +
                        "'\n")
                    self.status = -1
        elif cmd == "rstld":
            if not len(cmdlist_as) >= 3:
                ...
                try:
                    if int(
                            cmdlist_as[1]) in self.disks or int(
                            cmdlist_as[1]) == -1:
                        self.resetload_getfile(
                            int(cmdlist_as[1]), cmdlist_as[2])
                except ValueError:
                    self.outstr(
                        "ERROR: Invalid Integer in disk id! '" +
                        cmdlist_as[1] +
            ...
            if not len(cmdlist_as) >= 3:
                ...
                try:
                    if int(
                            ...
                            cmdlist_as[1]) == -1:
                        self.membackup(
                            int(cmdlist_as[1]), cmdlist_as[2], ga=True)
                except ValueError:
                    self.outstr(
                        "ERROR: Invalid Integer in disk id! '" +
                        cmdlist_as[1] +
            ...
            if not len(cmdlist_as) >= 2:
                self.outstr(
                    "ERROR: specify 'list [diskid] (optional pattern)'!\n")
                self.status = -2
            else:
                try:
                    if len(cmdlist_as) == 2:
            ...
            self.outstr(
                "ERROR: '" +
                cmd +
                "' is not valid/available in this mode!\n")
    ...
    def filelist(self, diskid, pattern):
        for did in self.disks:
            if diskid == did or diskid == -1:
                if self.disks[did] is None:
                    if diskid != -1:
                        self.outstr(
                            "ERROR:  drive index '" +
                            str(diskid) +
                            "' Not ready/no disk inserted.\n")
                        self.status = -5
                        return
                else:
            ...
            if diskid == did:
                if self.disks[did] is None:
                    ...
                    while addr <= libbaltcalc.mpi(9):
                        if ga:
                            diskfref.append(
                                [self.memsys2.INSTDICT[addr], self.memsys2.DATDICT[addr]])
                        else:
                            diskfref.append(
                                [self.memsys.INSTDICT[addr], self.memsys.DATDICT[addr]])
                        addr += 1
        if diskid not in self.disks and diskid != -1:
            self.outstr(
                "ERROR:  drive index '" +
            ...
            if diskid == did or diskid == -1:
                if self.disks[did] is None:
                    ...
                    if filename in self.disks[did].files:
                        self.resetload_restart(
                            self.disks[did].files[filename], ga=ga)
                        return
        if diskid not in self.disks and diskid != -1:
            self.outstr(
                "ERROR:  drive index '" +
    ...
    def resetload_restart(self, filelisting, ga=False):
        ...
        if ga:
            ...
            # reset CLI io buffers and params
            self.clireset(None, 0)
            # call special memory system resetload helper. (blanks RAM and
            # loads file into it)
