...
def assemble(pathx, syntaxonly=0, pfx="", exitonerr=1):
    # locate source file and extract destination basename and directory for
    # mainloop class.
...
class statinst:
    ...
    def p0(self, data, keyword, lineno):
        return 0, None
    # return length in words of memory. needed here for goto reference label
    # parsing!
...
class marker:
    ...
    def p0(self, data, keyword, lineno):
        ...
        return 0, "DEBUG Marker: '" + data + \
            "' source line: '" + str(lineno) + "'"
    ...
    def p2(self, data, keyword, gotos, lineno):
        ...
        return 0, "DEBUG Marker: '" + data + "' address: '" + \
            str(self.marks[lineno]) + "' source line: '" + str(lineno) + "'"
...
# basic instruction. literally any instruction that uses this
# automatically supports goto refrence carrot stntax
# (keyword;>gotorefrence)
class instruct:
    ...
    def p0(self, data, keyword, lineno):
        if data is None:
            ...
            if data[1:] in tcon.asm_chrtodat:
                ...
                return 1, keyword + ": Line: " + \
                    str(lineno) + ": unknown text character!"
            ...
            for char in data[3:]:
                ...
                if char not in bal27_valid:
                    return 1, keyword + ": Line: " + \
                        str(lineno) + ": invalid char in Septemvigesimal data string!"
            if bal27.b27toint(data[3:]) > 9841 or bal27.b27toint(
                    data[3:]) < -9841:
                return 1, keyword + ": Line: " + \
                    str(lineno) + ": Septemvigesimal Value out of range!"
            ...
            for char in data[2:]:
                ...
                if char not in bal9_valid:
                    return 1, keyword + ": Line: " + \
                        str(lineno) + ": invalid char in Nonary data string!"
            if bal9.b9toint(data[2:]) > 9841 or bal9.b9toint(data[2:]) < -9841:
                return 1, keyword + ": Line: " + \
                    str(lineno) + ": Nonary Value out of range!"
        ...
        return 0, None
    # return length in words of memory. needed here for goto refrence label
    # parsing!
    ...
    def p2(self, data, keyword, gotos, lineno):
        if data is None:
            ...
            try:
                ...
                return 1, keyword + ": Line " + \
                    str(lineno) + ": Nonexistant goto refrence!"
...
class rawinst:
    ...
    def p0(self, datafull, keyword, lineno):
        if datafull is None:
            return 1, keyword + ": Line: " + \
                str(lineno) + ": 'raw' requires TWO numeric arguments! raw;arga,argb"
        ...
        for data in datalist:
            if data.startswith(">"):
                ...
                if data[1:] in tcon.asm_chrtodat:
                    ...
                    return 1, keyword + ": Line: " + \
                        str(lineno) + ": unknown text character!(" + data + ")"
            elif data.startswith("27!"):
                data = data.upper()
                for char in data[3:]:
                ...
                for char in data[2:]:
                    ...
                    if char not in bal9_valid:
                        return 1, keyword + ": Line: " + \
                            str(lineno) + ": invalid char in Nonary data string!"
                if bal9.b9toint(data[2:]) > 9841 or bal9.b9toint(
                        data[2:]) < -9841:
                    return 1, keyword + ": Line: " + \
                        str(lineno) + ": Nonary Value out of range!"
            elif data.startswith("10x"):
                try:
                    ...
                    return 1, keyword + ": Line: " + \
                        str(lineno) + ": decimal int syntax error! (" + data + ")"

            else:
                if len(data) > 9:
                    return 1, keyword + ": Line: " + \
                        str(lineno) + ": string too large! (" + data + ")"
                for char in data:

                    if char not in tritvalid:
                        return 1, keyword + ": Line: " + \
                            str(lineno) + ": invalid char in ternary data string! (" + data + ")"
        return 0, None
    # return length in words of memory. needed here for goto refrence label
    # parsing!
        ...
        for data in datalist:
            if data.startswith(">"):
                try:
                    ...
                    return 1, keyword + ": Line " + \
                        str(lineno) + ": Nonexistant goto refrence! (" + data + ")"
...
class includetas0:
    ...
    def p0(self, datafull, keyword, lineno):
        if datafull is None:
            return 1, keyword + ": Line: " + \
                str(lineno) + ": 'raw' requires tas0 filename as argument!"
        # load tas0 file.
        tas0fobj = iofuncts.loadtrom(
            datafull, ext=".tas0", exitonfail=0, dirauto=1)
        if tas0fobj is None:
            return 1, keyword + ": Line: " + \
                str(lineno) + ": Unable to load tas0 file!"
        # init mainloop
        tas0main = mainloop(
            tas0fobj,
            ".",
        ...
        if tas0main.p0():
            return 1, keyword + ": Line: " + \
                str(lineno) + ": tas0 pass 0: Syntax Error!"
    ...
    def p1(self, data, keyword, lineno, addr, gotos):
        ...
        # set address start to current main addr for proper handling of tas0
        # adresses.
    ...
    def p2(self, datafull, keyword, gotos, lineno):
        ...
        if tas0main.p2():
            return 1, keyword + ": Line: " + \
                str(lineno) + ": tas0 pass 2: Syntax Error!"
        return 0, None
    # should return two signed ints or btint objects.
...
class includetas0_as:
    ...
    def p0(self, datafull, keyword, lineno):
        if datafull is None:
            return 1, keyword + ": Line: " + \
                str(lineno) + ": requires tas0 filename as argument!"
        try:
            filed, name = datafull.split(",")
        except ValueError:
            return 1, keyword + ": Line: " + \
                str(lineno) + ": requires [tas0 filename],[name] as arguments!"
        if name == "":
            return 1, keyword + ": Line: " + \
                str(lineno) + ": missing/blank include name!"
        # load tas0 file.
        tas0fobj = iofuncts.loadtrom(
            filed, ext=".tas0", exitonfail=0, dirauto=1)
        if tas0fobj is None:
            return 1, keyword + ": Line: " + \
                str(lineno) + ": Unable to load tas0 file!"
        # init mainloop
...
class nspacevar:
    ...
    def p0(self, data, keyword, lineno):
        if not len(keyword) > 2:
            return 1, keyword + ": Line: " + \
                str(lineno) + ": No variable name after prefix! (v>varname)"
        if data is None:
            return 0, None
        elif data.startswith(">"):
            return 0, None
            ...
            for char in data:
                ...
                if char not in tritvalid:
                    return 1, keyword + ": Line: " + \
                        str(lineno) + ": invalid char in ternary data string!"
        return 0, None
    # return length in words of memory, and if any, custom namespace entries.
    # needed here for goto refrence label parsing!
    ...
    def p1(self, data, keyword, lineno, addr, gotos):
        if data is None:
            ...
            try:
                ...
                sys.exit("ERROR: v>: namespace variable: '" +
                         data[1:] + "does not exist. line: '" + lineno + "'")
        else:
    ...
    def p2(self, data, keyword, gotos, lineno):
        ...
        return 0, None
    # should return two signed ints or btint objects, or if no output, an
    # empty list.
...
class mainloop:
    def __init__(
        ...
        self.instlist = [instruct(["null"], 0),
                         ...
                         # ABSOLUTE and inverted absolute instructions
                         instruct(["abs1"], -9834),
                         instruct(["abs2"], -9833),
                         instruct(["nabs1"], -9832),
                         instruct(["nabs2"], -9831),
    ...
    def headload(self):
        ...
        for line in self.fileobj:
            ...
            if line.startswith("head-nspin="):
                ...
                nspobj = iofuncts.loadtrom(
                    nspname,
                    ext=".nsp",
                    exitonfail=1,
                    exitmsg="ERROR: header: head-nspin: nonexistant nsp file! '" +
                    nspname +
                    "'",
                    dirauto=1)
                for linex in nspobj:
            ...
            if line.startswith("head-rname="):
                ...
                if len(self.romdestname) == 0:
                    sys.exit(
                        self.pfx +
                        "ERROR: header: head-rname: name must be at least 1 character long.")
                for char in self.romdestname:
                    if char not in nameallowed:
                        sys.exit(
                            self.pfx +
                            "ERROR: header: head-rname: invalid char '" +
                            char +
                            "' in name: '" +
                            self.romdestname +
                            "'")
            ...
            if line.startswith("head-nspname="):
                ...
                if len(self.nspdestname) == 0:
                    sys.exit(
                        self.pfx +
                        "ERROR: header: head-nspname: name must be at least 1 character long.")
                for char in self.nspdestname:
                    if char not in nameallowed:
                        sys.exit(
                            self.pfx +
                            "ERROR: header: head-nspname: invalid char '" +
                            char +
                            "' in name: '" +
                            self.nspdestname +
                            "'")
                print(
                    self.pfx +
                    "Header: nsp name override: '" +
                    self.nspdestname +
                    "'")

        # header framework for future alternate assemble modes. i.e. different
        # starting offsets, etc.
        if self.mode == "trom":
            ...
            sys.exit(
                self.pfx +
                "ERROR: header: head-mode: Invalid mode: '" +
                mode +
                "'")
        # set assembler rom output filename.
        self.romoutput = os.path.join(
            self.basepath, self.romdestname + self.destext)
        if self.nsp == 1:
            self.nspfile = os.path.join(
                self.basepath, self.nspdestname + '.nsp')
            self.nspdict = {}
        else:
            self.nspfile = None
            self.nspdict = None
                    ...
                    if keyword in inst.keywords:
                        ...
                        if retlist[0] == 1:
                            ...
                            if comment is not None:
                                print(
                                    self.pfx +
                                    "----NOTICE----: Comment from faulty line: \n" +
                                    self.pfx +
                                    "'" +
                                    comment +
                                    "'")
                        ...
                        for pattern in inst.prefixes:
                            if keyword.startswith(pattern):
                                ...
                                if retlist[0] == 1:
                                    if retlist[1] is not None:
                                        print(
                                            self.pfx + "Syntax Error: " + retlist[1])
                                    else:
                                        print(self.pfx + "Syntax Error!")
                                    if comment is not None:
                                        print(
        ...
        for line in self.fileobj:
            ...
            if not line.startswith("#") and not line.startswith("head-"):
                ...
                for inst in self.instlist:
                    # normal keywords
                    if keyword in inst.keywords:
                        length, nspaceadd = inst.p1(
                            data, keyword, lineno, addr, self.gotos)
                        if glabel != "" and glabel is not None:
                            self.gotos[glabel] = addr
                            if self.nspfile is not None and not glabel.startswith(
                                    "."):
                                self.nspdict[glabel] = addr
                        if nspaceadd != {}:
                            self.gotos.update(nspaceadd)
                            if self.nspfile is not None and inst.nsp:
                ...
                for inst in self.instlist:
                    ...
                    if keyword in inst.keywords:
                        ...
                        for pattern in inst.prefixes:
                            if keyword.startswith(pattern):
                                retlist = inst.p2(
                                    data, keyword, self.gotos, lineno)
                                if retlist[0] == 1:
                                    if retlist[1] is not None:
                                        print(
                                            self.pfx + "Syntax Error: " + retlist[1])
