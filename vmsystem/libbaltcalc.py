...
# note that values may not be exact. this is due to that the libbaltcalc
# currently handles integers only.
...
# returns "0" either if neither or both inputs equal the pollarity line
# (polarset)
    ...
    def __init__(self, stringint):
        ...
        if isinstance(stringint, int):
            ...
            try:
                ...
                self.intval = BTTODEC(
                    str(stringint).replace("p", "+").replace("n", "-"))
    ...
    def changeval(self, newval):
        if isinstance(newval, int):
            ...
            try:
                ...
                self.intval = BTTODEC(
                    str(newval).replace("p", "+").replace("n", "-"))
    # addition
