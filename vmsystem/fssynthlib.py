...
def makesquare(freq):
    return array.array('i', [ewchunk(foobsin_square(
        2.0 * math.pi * freq * t / mixrate)) for t in xrange(0, int(mixrate / freq))])


def makesine(freq):
    return array.array('i', [ewchunk(foobsin_sine(
        2.0 * math.pi * freq * t / mixrate)) for t in xrange(0, int(mixrate / freq))])
...
def makedistangent(freq):
    freq = freq / 2.0
    return array.array('i', [ewchunk(foobsin_tangent_distort(
        2.0 * math.pi * freq * t / mixrate)) for t in xrange(0, int(mixrate / freq))])
...
def makepulse(freq, duty=0.5):
    ...
    return array.array('i', [ewchunk(pulser(t, swtime))
                       for t in xrange(0, int(mixrate / freq))])


# generate sawtooth wave.
def makesaw(freq):
    sawramp = 1.0 / (mixrate / float(freq))
    return array.array('i', [ewchunk(((sawramp * t) * 32765) - 15000)
                       for t in xrange(0, int(mixrate / freq))])
...
def maketri(freq):
    ...
    return array.array('i',
                       [ewchunk((trihelp(triramp,
                                         trifall,
                                         rangesize,
                                         t) * 36765) - 17000) for t in xrange(0 + quarter,
                                                                              int(mixrate / freq) + quarter)])
...
def maketrisl(freq, duty=0.5):
    ...
    return array.array('i',
                       [ewchunk((trihelp_sl(triramp,
                                            trifall,
                                            rangesize,
                                            t,
                                            trirampfall) * 36765) - 17000) for t in xrange(0 + quarter,
                                                                                           int(mixrate / freq) + quarter)])
...
def makenoise():
    return array.array('i', [ewchunk(
        (random.randint(0, 1) * 24765) - 12000) for t in xrange(0, int(mixrate))])

# stereo signed 16bit sound data encoder. intended for use in array generator.
# takes value of foobsin calculation and returns 2 joined, signed 16bit
# datavalues for wave data.
...
def test8(freq=110):
    print(
        "Sliding trinagle synthesis test (sweep) (test 1) Freq: '" +
        str(freq) +
        "'")
    chan = pygame.mixer.Channel(1)
    chan.set_volume(1, 1)
...
def test6(freq=110):
    print(
        "Distorted tangent wave synthesis test (test 6) Freq: '" +
        str(freq) +
        "'")
