    ...
    try:
        ...
        if vmconf.check_has_vmconf(romfile):
            ...
            romfile = iofuncts.findtrom(
                cfg_dict["rom0"], ext=".trom", exitonfail=1, dirauto=1)
            if cfg_dict["rom1"] is not None:
                garom = iofuncts.findtrom(
                    cfg_dict["rom1"], ext=".trom", exitonfail=1, dirauto=1)
        ...
        ttywin.addstr(
            maxy - 1,
            0,
            "SBTCVM Curses frontend. SBTCVM Gen2-9 v2.1.0")
        ttywin.scrollok(1)
        ttywin.scroll(1)
        ttywin.refresh()
        ...
        while progrun:
            ...
            clcnt += 1
            # project when the next cycle should start, then subtract current
            # time.
