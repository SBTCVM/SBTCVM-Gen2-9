...
def InputFileLoader(arg, dirauto=1):
    ...
    if fname is not None and "^" not in arg:
                ...
                for char in tcon.allchars:
                    if char.dataval == trval and trval != 0 and trval != 2:
                        ...
                        if char.dataval == 1 and isstring == 1:
                            ...
                            # if at start of string, start with pass no. and
                            # starting address.
                            if isstring == 0:
                                isstring = 1
                                pstring = str(searchpass) + ", At " + \
                                    str(mempos).rjust(5) + " :"
                            pstring += (char.bldumpstr)
                            stringcnt += 1
                        trischar = 1
...
def b27_char(fileobj, start, end):
    ...
    for line in fileobj:
        ...
        if widcnt == 9:
            if mempos in xrange(start,
                                end + 1) or mempos - 8 in xrange(start,
                                                                 end + 1) or start in xrange(mempos - 8,
                                                                                             mempos) or end in xrange(mempos - 8,
                                                                                                                      mempos):
                print(inst + "|" + inst_char + "||" + data + "|" + data_char +
                      "||" + str(mempos - 8).rjust(6) + " TO" + str(mempos).rjust(6))
            inst = ""
            data = ""
            inst_char = ""
            data_char = ""
...
def romdumpver(fileobj, start, end, n0p=0):
    ...
    for line in fileobj:
        ...
        if mempos in xrange(start, end + 1):
            ...
            pstring += bal27.inttob27(int(trval0)).rjust(6) + \
                bal27.inttob27(int(trval1)).rjust(4)
        ...
        if "^" in arg:
            ...
            if diskfilename in dsk.files:
                ...
                if diskfilename.lower().endswith(".txe"):
                    print(
                        "----VDI FileType: TXE:  Ternary Executable. [RAW G2-9 format]")
                else:
                    print("----VDI FileType: (?):  Unknown extention.")
                print("File Size: " +
                      g2com.nonetformatted_smart(len(dsk.files[diskfilename])))
                print("--------")
        else:
            if fname2.lower().endswith(".tdsk1"):
                ...
                for filex in dsk.files:
                    print("    " + filex.ljust(20) + " " +
                          g2com.nonetformatted_smart(len(dsk.files[filex])))
                    disksize += len(dsk.files[filex])
                print(
                    "Disk size (virtual): " +
                    g2com.nonetformatted_smart(disksize))
                print("--------")
    else:
        if fname is None:
...
if __name__ == "__main__":
    ...
    if cmd in [
        ...
        try:
            ...
            sys.exit(
                "Error: ranged mode requires 2, space-separated signed decimal addresses.")
    else:
        try:
...
see readme.md for more information and licensing of media.
  ...
  ''')
        ...
        romdump_stringfind_interlaced(
            InputFileLoader(
                arg, dirauto=1), start, end)
    elif cmd in ["-d"]:
